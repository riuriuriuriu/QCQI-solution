...
\begin{ex}
    \label{ex8.18}
    CP写像$\qo　\colon \linearopset{\mathbb{C}^n} \to  \linearopset{\mathbb{C}^m}$が    \begin{align*}
        \sum_{k=1}^l E_k^\dagger E_k \le I
    \end{align*}
    を満たす演算要素$\{ E_k \colon \mathbb{C}^n \to \mathbb{C}^m\}_{k=1}^{l}$を用いて, 
    \begin{align*}
        \qo \left( \rho \right)
        = \sum_{k=1}^{l} E_k \rho E_k^\dagger
    \end{align*}
    で定義されるとすると,
    \begin{align*}
        \tr \left[ \qo \left( \rho^{n} \right)\right]
        =
        \sum_{k=1}^{l}  \tr \left[ E_k \rho^n E_k^\dagger \right]
        =
        \sum_{k=1}^{l}  \tr \left[\rho^n E_k^\dagger E_k \right]
        \le \tr(\rho^n) .
    \end{align*}
\end{ex}
\begin{ex}
